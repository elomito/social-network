...
func CreateNotificationHandler(svc *services.NotificationService) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil { http.Error(w, "invalid recipient", http.StatusBadRequest); return }
		var initiator *uuid.UUID
		if payload.InitiatorID != "" {
			if uid, err := uuid.Parse(payload.InitiatorID); err == nil { initiator = &uid }
		}
		...
		if err != nil { http.Error(w, "failed to create", http.StatusInternalServerError); return }
...
func ListNotificationsHandler(svc *services.NotificationService) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if !ok || uidStr == "" {
			http.Error(w, "unauthenticated", http.StatusUnauthorized); return
		}
		userID, err := uuid.Parse(uidStr)
		if err != nil { http.Error(w, "invalid user", http.StatusBadRequest); return }

		limit := 50
		if lstr := r.URL.Query().Get("limit"); lstr != "" {
			if l, err := strconv.Atoi(lstr); err == nil { limit = l }
		}
		notifs, err := svc.ListForUser(r.Context(), userID, limit)
		if err != nil { http.Error(w, "failed to list", http.StatusInternalServerError); return }
		w.Header().Set("Content-Type", "application/json")
...
func MarkNotificationReadHandler(svc *services.NotificationService) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := svc.MarkRead(r.Context(), id); err != nil { http.Error(w, "failed", http.StatusInternalServerError); return }
