...
export default function NotificationItem({ notification, onAccept, onDecline }) {
  return (
    <div className="flex items-start justify-between border-b bg-white p-3 transition-colors hover:bg-gray-50">
      ...
      <div className="flex shrink-0 items-center gap-2">
        ...
        {notification.type === 'event_created' && notification.groupId && (
          <Link
            ...
            className="rounded border border-indigo-200 bg-indigo-50 px-2 py-1 text-xs font-semibold text-indigo-700 transition-colors hover:bg-indigo-100"
          >
            View Event
          </Link>
        )}
